...
import exps.cariv.global.exception.ErrorResponse;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (token != null) {
            try {
                // 토큰 검증 + Claims 파싱을 한 번에 (이중 파싱 방지)
                Claims claims = jwtTokenProvider.validateAndGetClaims(token);

                // access 토큰만 요청 인증에 사용 가능
                ...
                // SecurityContext에 인증정보 넣기 (Claims 재사용)
                var authentication = jwtTokenProvider.getAuthentication(claims);
                Object principal = authentication.getPrincipal();
                if (principal instanceof CustomUserDetails cud) {
                    // MASTER 위임 토큰(createAccessTokenForTenant)은 tenant 전환을 허용한다.
                ...
                // JwtTokenProvider는 TOKEN_*을 CustomException으로 감싸서 던진다.
